...
# Create timestamp
if [[ ! -f $CALIPTRA_SS_ROOT/.github/workflow_metadata/pr_timestamp ]]; then
    echo "Error, file not found: $CALIPTRA_SS_ROOT/.github/workflow_metadata/pr_timestamp"
    exit 1
fi
timestamp=$(date +%s)
echo "Submitting timestamp [${timestamp}]"
echo -n ${timestamp} > $CALIPTRA_SS_ROOT/.github/workflow_metadata/pr_timestamp

# Create hash
hash=$($CALIPTRA_SS_ROOT/.github/scripts/file_hash.sh $CALIPTRA_SS_ROOT $CALIPTRA_SS_ROOT/.github/workflow_metadata/file_list.txt)
...
if [[ ! -f $CALIPTRA_SS_ROOT/.github/workflow_metadata/pr_hash ]]; then
    echo "Error, file not found: $CALIPTRA_SS_ROOT/.github/workflow_metadata/pr_hash"
    exit 1
fi
echo "Submitting hash [${hash}]"
echo -n ${hash} > $CALIPTRA_SS_ROOT/.github/workflow_metadata/pr_hash

# Clean up
rm $CALIPTRA_SS_ROOT/.github/workflow_metadata/file_list.txt
