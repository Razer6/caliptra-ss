...
# 'target' is the (gitignored) cargo build directory of tools/rom_patch, which
# also holds everything tools/rom_patch/e2e/run.sh generates.
exclude_dir='{uvmf*,.git,cmark,__pycache__,templates,docs,doc,third_party,riscv_core,tlul,ISSUE_TEMPLATE,target}'
...
exclude_csr='*_csr*.*'
# Cargo.lock is regenerated verbatim by cargo, which drops any comment header.
exclude_file='{sglint_waivers,pr_hash,pr_timestamp,.gitmodules,.git-comodules,.gitignore,spyglass_lint.policy,ascent.ctl,CODEOWNERS,clp_mapfile,c_sample.c,Cargo.lock}'
# excluding OT Files.
